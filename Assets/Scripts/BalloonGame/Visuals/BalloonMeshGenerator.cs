...
public static class BalloonMeshGenerator
{
    ...
    private static Mesh Generate()
    {
        ...
        for (int ring = 1; ring <= rings; ring++)
        {
            ...
            float polar = Mathf.Lerp(0.02f, Mathf.PI - 0.02f, v);
            ...
            if (v > 0.85f)
            {
                radius *= Mathf.Lerp(1f, 0.08f, Mathf.InverseLerp(0.85f, 1f, v));
            }
        ...
        // Bottom cap vertex — smooth close, no tie stub
        int bottomCapIndex = vertices.Count;
        vertices.Add(new Vector3(0f, -bodyHeight * 0.48f, 0f));
        ...
        // Bottom cap — connect last ring to single bottom vertex
        int lastRingStart = 1 + (rings - 1) * segments;
        for (int segment = 0; segment < segments; segment++)
        {
            int next = segment == segments - 1 ? 0 : segment + 1;
            triangles.Add(lastRingStart + segment);
            triangles.Add(bottomCapIndex);
